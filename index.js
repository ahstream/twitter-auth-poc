...
const { getOAuthRequestToken, getOAuthAccessTokenWith, oauthGetUserById } = require('./oauth-utilities');
...
async function main() {
  ...
  app.get('/twitter/callback', async (req, res) => {
    try {
      ...
      console.log('getOAuthAccessTokenWith:', { oauthAccessToken, oauthAccessTokenSecret, results });

      const { user_id: userId, screen_name: screenName } = results;
      ...
      res.cookie('twitter_user_id', userId, { maxAge: 900000, httpOnly: true });
      res.cookie('twitter_screen_name', screenName, { maxAge: 900000, httpOnly: true });
      console.log('user succesfully logged in with twitter', screenName);

      req.session.save(() => res.redirect('/'));
    } catch (e) {
      console.error('Error in /twitter/callback:', e);
    }
  });
}
